...
#define DT_DRV_COMPAT curstom_st7789v
...
static int st7789v_init(const struct device *dev)
{
	...
	if (config->bl_ctrl_gpio.port != NULL) {
		if (!device_is_ready(config->bl_ctrl_gpio.port)) {
			LOG_ERR("Backlight GPIO device not ready");
			return -ENODEV;
		}

		if (gpio_pin_configure_dt(&config->bl_ctrl_gpio, GPIO_OUTPUT_INACTIVE)) {
...
#define ST7789V_INIT(inst)								\
	static const struct st7789v_config st7789v_config_ ## inst = {			\
		...
		.bl_ctrl_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, bl_ctrl_gpios, {}),		\
